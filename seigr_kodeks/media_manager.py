...
def insert_media(book_path, media_type, file_types):
    ...
    file_path = filedialog.askopenfilename(title=f"Select {media_type} File", filetypes=[(f"{media_type} Files", file_types)])
    ...
    if media_type == "Image":
        ...
        markdown_syntax = f"<video controls><source src='media/{file_name}' type='video/mp4'></video>"
    else:
