...
def ensure_book_structure(book_path):
    ...
    if not os.path.exists(book_metadata_path):
        book_metadata = {"title": os.path.basename(book_path), "chapters": [], "media": []}
...
def add_chapter(book_path, chapter_title):
    ...
    if chapter_filename not in [ch["filename"] for ch in metadata["chapters"]]:
        ...
        metadata["chapters"].append({"title": chapter_title, "filename": chapter_filename})
...
def reorder_chapters(book_path, old_index, new_index):
    ...
    if 0 <= old_index < len(metadata["chapters"]) and 0 <= new_index < len(metadata["chapters"]):
...
def organize_files(book_path):
    ...
    for file in os.listdir(book_path):
        ...
        if file.endswith(".md") and file not in [ch["filename"] for ch in metadata["chapters"]]:
            shutil.move(file_path, os.path.join(chapters_dir, file))

        elif file.lower().endswith((".png", ".jpg", ".jpeg", ".gif", ".mp3", ".wav", ".mp4", ".webm", ".ogg")):
